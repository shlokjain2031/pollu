...
def read_overview_stats(src, max_dim=1024):
    ...
    out_w = w // scale
    arr = src.read(
        1, out_shape=(out_h, out_w), resampling=rasterio.enums.Resampling.bilinear
    )
    return arr, scale
...
def heuristic_dsm_decision(meta, arr, grad_mag, local_stats):
    ...
    if frac_high > 0.25:
        decisions.append(
            f"{frac_high:.2f} fraction of local windows have range>2m (buildings/trees likely)"
        )
...
def make_report(path: Path, args) -> int:
    ...
    with rasterio.open(path) as src:
        ...
        print(
            "\nGradient stats (overview): mean {:.4f}, std {:.4f}".format(
                float(np.nanmean(grad_mag)), float(np.nanstd(grad_mag))
            )
        )

        # sample local windows at full res
        print("\nSampling local windows (full resolution)...")
        local_stats = sample_local_stats(
            src, n_samples=args.samples, window_size=args.window
        )
        if local_stats:
            ls_arr = np.array([s["range"] for s in local_stats])
            print(" local windows sampled:", len(local_stats))
            print(
                " local range: mean {:.2f} m, median {:.2f} m, p90 {:.2f} m".format(
                    float(np.mean(ls_arr)),
                    float(np.median(ls_arr)),
                    float(np.nanpercentile(ls_arr, 90)),
                )
            )
        else:
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=80,
        help="number of local windows to sample (default 80)",
    )
    parser.add_argument(
        "--window",
        type=int,
        default=11,
        help="window size in pixels for local stats (odd number, default 11)",
    )
