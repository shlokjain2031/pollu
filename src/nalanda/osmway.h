...
namespace pollu {
    namespace nalanda {
        struct OSMWay {
            ...
            uint8_t use_        : 6;  // footway, steps, alley, cycleway, etc.
            uint32_t lanes_      : 4;
            ...
            void set_use(const Use u) { use_ = static_cast<uint8_t>(u); }
            uint8_t use() const { return use_; }

            void set_lanes(uint32_t l) { lanes_ = l; }
            uint32_t lanes() const { return lanes_; }

            void set_surface(Surface s) { surface_ = static_cast<uint8_t>(s); }
